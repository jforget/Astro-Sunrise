...
Sun's position at 20:17:31. This virtual solar noon occurs at 12:xx:xx.
With this reference, we apply the Earth's rotation and we get a second
value for sunset, 20:xx:xx.

If a third iteration is necessary, we determine the virtual solar noon 
that corresponds to the Sun's position at 20:xx:xx. This new virtual solar
noon occurs at 12:yy:yy. And one more time we apply the Earth's rotation
and we obtain a third value for sunset, 20:yy:yy.
...
During iteration 2, between the real solar noon 12:zz:zz and the time given
by iteration 1, 20:17:31, the Sun orbitates with its real speed of 1.0166 degree per day.
Then, at 20:17:31, the Sun freezes in its track and after that, there is only
the Earth's spin and the sunset occurs some time after, at 20:xx:xx.

During iteration 3, between the real solar noon 12:zz:zz and the time given
by iteration 2, 20:xx:xx, the Sun orbitates with its real speed 1.0166 degree per day.
Then at 20:xx:xx, it freezes, letting the Earth continue its spin. And sunset 
happens a few instants later, at 20:yy:yy. So, there are 8 h x mn x s when we use
the Sun's real orbital speed and y seconds when we use an obviously wrong orbital
speed. In the end, it is better than the basic algorithm, which uses an approximate
but still wrong orbital speed, but for the whole span of 8 h, x mn and x s.
