...
Copyright (c) 2017, 2018 Jean Forget. All rights reserved.
...
of sunrise, sunset or true solar noon.
...
the true solar noon or use C<Astro::Sunrise> (basic mode) for
sunrise or sunset.
...
C<altitude> parameter of C<Astro::Sunrise>'s functions.
...
C<Astro::Sunrise>'s value, which is 35' or 0.583 degree and
...
If not already done, hit C<< <F2> >> to open the configuration
menu. In this menu, select the C<Plugins> tab, then C<Time zone>. With this,
you can select a UTC display instead of a display using the 
computer's local timezone. Do not forget to save the configuration
upon exit.

=item *

Look for the Sun. The window is activated by C<< <F3> >>.
...
In several examples below, I have chosen a location at
51° 28' 40" N, 5" E and a height of 27 m. The longitude and 
latitude are the values given by Wikipedia for the Greenwich
Observatory and the height is nothing more than the previously
entered height. This has no relation with the observatory's real
height, or this is a lucky coincidence.
...
here the variation is multiplied by 8 and the solar noon varies from 10h, or even less,
to 14h. But without this stretching, you would not have seen anything.
...
and a I<virtual> altitude of 42°47'31".
...
And instead of using the example of Greenwich, we take its symmetrical point
with respect to the Equator. This point is not far from Bouvet Island in
South Atlantic. And the date is 4th January, 2018.

The basic algorithm begins by computing the day's true solar noon.
On 4th Jan, the true solar noon at Anti-Greenwich happens at 12:04:59
(actually, same as Greenwich).

Then we apply both Earth's spin (360.986 degrees per day) and 
the movement of a VHS ("virtual homocinetic sun"), that is, 0.986 degrees per
...
So on 4th Jan, the angle between noon and sunset is 123.4° (123° 24' 25"). 
We need 8.2271 hours (8 h 13 mn 35 s) to run this angle and the sunset for
the VHS occurs at 20:18:34.

=head2 Implementation of Precise Algorithm

With the precise algorithm, we keep separate Earth's spin (360.986 degrees
per day) and the Sun's rotational speed around Earth. In additionn, this
rotational speed is the real speed, spanning from 0.9552 to 1.0166 
degree per day.

First iteration. We start from the true solar noon and we apply Earth's spin
...
On 4th Jan, this first value is 20:17:31.

This value is noticeably worse than the value from the basic algorithm because
it does not take into account the Sun's orbital speed around the Earth, even
approximately. But we will do better with the second iteration.

For iteration 2, we determine the virtual solar noon that corresponds to the
Sun's position at 20:17:31. This virtual solar noon occurs at 12:05:06.
With this reference, we apply the Earth's rotation and we get a second
value for sunset, 20:17:24.

If a third iteration is necessary, we determine the virtual solar noon 
that corresponds to the Sun's position at 20:17:24. This new virtual solar
noon occurs at 12:05:06. And one more time we apply the Earth's rotation
and we obtain a third value for sunset, 20:17:24, differing from the previous
value by less than a second.
...
by iteration 1, 20:17:31, the Sun orbitates with its real speed of 1.0166 degree per day
while the Earth spins at 360.986 degrees per day.
Then, at 20:17:31, the Sun freezes in its track and after that, we adjust the
position with only the Earth's spin to reach the required altitude.
In this cas, we overshot the required altitude, so Earth spins backward
while going back to the past.
And the sunset occurs at 20:17:24.

During iteration 3, between the real solar noon 12:04:56 and the time given
by iteration 2, 20:17:24, the Sun orbitates with its real speed 1.0166 degree per day.
Then at 20:17:24, it freezes, letting the Earth continue its spin. And sunset 
happens less than a second later (or earlier), at 20:17:24. So, there are 8 h 12 mn 28 s when we use
the Sun's real orbital speed and less than one second when we use an obviously wrong orbital
speed. In the end, it is better than the basic algorithm, which uses an approximate
but still wrong orbital speed, but for the whole span of 8 h, 12 mn and 28 s.
...
direction of I<decreasing> right ascension valude. So, when the true solar noon is
