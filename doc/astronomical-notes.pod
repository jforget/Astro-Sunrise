...
TO BE COMPLETED: numerical values
...
in a geocentric model, that is perihelion in an heliocentric model) and a spped
of 0.9552 degree per day in July at apogee (or aphelion).

Both phenomena add to each other, so the Sun's angular speed measured on the 
equatorial plane is not constant. Its mean value is 0.986 degree per day, but
it varies
TO BE COMPLETED
