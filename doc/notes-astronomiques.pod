...
présente dans la GPL et dans la licence I<Artistic> s'appliquent non seulement
...
L<Astro::Sunrise>,     il     documente    également     le     module
L<DateTime::Event::Sunrise>, dont le noyau (calculs astronomiques) est
...
en déphasage avec le module L<Astro::Sunrise>.
...
L<décrit succintement|https://www.stjarnhimlen.se/comp/riset.html#3> 
par Paul Schlyter
dans son site web et qu'il n'a pas mis à disposition du public
sous une forme compilable et exécutable (contrairement à la
L<version simple sans itération|https://www.stjarnhimlen.se/comp/sunriset.c>).
...
À propos des minutes et des S<secondes :> il y a un problème dans la mesure où
...
formats différents pour favoriser la distinction entre ces S<cas :>
S<« 12:28:35 »> pour le repérage d'un instant, S<« 2 h 28 mn 35 s »> pour les
durées et S<« 59° 28' 35" »> pour les angles (latitudes, longitudes et autres).
Donc, même s'il n'y a pas d'indication de degré ou d'heure, vous pourrez faire
la distinction entre un angle S<« 28' 35" »> et une durée S<« 28 mn 35 s »>.
...
Déterminer une valeur approximative de l'heure S<recherchée :> 12:00 pour le midi vrai,
...
être compatible avec le paramètre C<alt> (hauteur) des fonctions de L<Astro::Sunrise>.
...
Presser C<< <F6> >> pour spécifier le lieu d'observation. Prendre un
...
S<« utiliser> un fuseau horaire S<personnalisé »> et choisir le fuseau
UTC.

Dans la version 0.18.0, il est possible d'utiliser S<« Royal>
Observatory S<(Greenwich) »>. Dans la version précédente, j'ai dû
créer un lieu d'observation à 51° 28' 40" de latitude nord,
5" de longitude est et S<27 m> de hauteur. La longitude et la latitude
...
mais un paramètre global de l'application. Il fallait taper C<< <F2> >>
pour avoir le menu de configuration. Dans ce menu, il fallait
choisir l'onglet C<Plugins>, puis C<Fuseau horaire>. Il était alors
...
L<S<« Cosmic> S<Story »>|https://www.savoir-sans-frontieres.com/JPP/telechargeables/Francais/cosmic_story.htm>.
...
Le présent paragraphe et le suivant se basent sur le code Perl S<suivant :>
...
par une fraction de S<seconde :> 16.0731642391519 au lieu de 16.0731615074431.
...
=head2 Que s'est-il passé au printemps 2020 ?

Remontons un  peu plus loin. En  janvier 2019, j'ai publié  la version
0.98 de  L<Astro::Sunrise>, avec l'implémentation du  calcul itératif.
...
L<DateTime::Event::Sunrise>, j'estimais  que ce n'était pas  encore le
moment.

Puis, en avril 2020, un utilisateur  a créé un ticket expliquant qu'il
avait comparé  les résultats  de L<DateTime::Event::Sunrise>  avec des
sites web de référence et que les résultats n'étaient pas très précis.
Certes, le module L<DateTime::Event::Sunrise> n'était pas au niveau de
L<Astro::Sunrise>, donc  la remarque  était justifiée. Cela  m'a donné
...
L<DateTime::Event::Sunrise>  au même  niveau que  L<Astro::Sunrise> et
pour utiliser des données de tests correctes, issues des sites web que
l'utilisateur avait pris pour références.

Seulement voilà,  l'algorithme précis  hérité de  L<Astro::Sunrise> et
...
L<DateTime::Event::Sunrise>  avec   la  valeur   15,  alors   que  mes
explications  donnaient  une  bonne  raison pour  utiliser  la  valeur
15,04107.

=head3 Qu'est-ce qui cloche ?
...
réalité, le  programme calcul le  midi solaire virtuel  pour 16:04:11,
qui donne 12:05:01 et qu'ensuite il utilise le Soleil à sa position de
12:05:01.

Comment le vérifier ?  Pour cela, il faudrait que je  puisse S<« créer »>
...
=head1 Examen détaillé des paramètres
...
=head2 Choix de l'algorithme, paramètre C<precise>

Q : Quand est-il préférable de choisir l'algorithme précis ?

R : La réponse courte, c'est S<« jamais »>. La réponse développée est la suivante.
...
Q: Donc, utiliser un paramètre C<alt> à -0,583 et un paramètre C<upper_limb> à 0 est stupide ?

A: Non, je dirais que c'est juste inhabituel.  Comme le signale Paul Schlyter
L<dans son site web|http://www.stjarnhimlen.se/comp/riset.html#2>, 
les almanachs nationaux de Suède définissent le lever et le coucher du soleil comme
l'instant où le I<centre> du disque solaire atteint l'horizon optique, alors que le consensus
international les définit comme étant l'instant où le I<sommet> du disque solaire atteint l'horizon
optique.

Q: En revanche, utiliser un paramètre C<alt> à -0,833 et un paramètre C<upper_limb> à 1 est stupide, n'est-ce pas ?
...
=item * 0 pour la position du point d'observation par rapport au paysage environnant,

=item * -0,583° ou -35' pour la réfraction,

=item * -0,25° ou -15' pour le rayon du disque solaire, ou 0 si l'on alimente C<upper_limb> à 1.
...
=item * -0,25° (-15') pour la position du point d'observation par rapport au paysage environnant,

=item * -0,583° ou -35' pour la réfraction,

=item * pour le rayon du disque solaire, 0 si l'on alimente C<upper_limb> à 1, ou bien -0,25° (-15') si C<upper_limb> vaut 0.

=back

C'est donc parfaitement valide, mais douteux parce que cela ne correspond pas à une situation habituelle.

Q: Compte tenu de ce que tu as dit pour le paramètre C<precise>, est-ce que la différence entre 
C<< alt => -0.833, upper_limb => 0 >> et C<< alt => -0.583, upper_limb => 1 >> est sensible ?
...
C<< alt => -0.833, upper_limb => 0 >>, le coucher du soleil se produit lorsque
...
C<< alt => -0.833, upper_limb => 0 >> indique que le soleil reste en-dessous de l'horizon,
tandis que le calcul avec C<< alt => -0.583, upper_limb => 1 >> 
indique un lever du soleil à 12:45:11 et le coucher à 13:05:53.

=head2 Année, paramètre C<year>

Q : Pourquoi L<Astro::Sunrise> a-t-il besoin de l'année pour calculer le lever
...
modifier L<Astro::Sunrise> pour afficher les secondes.
...
l'année de la I<nuit polaire>. Vous trouverez un exemple de pseudo-analemme
arctique dans le paragraphe sur le paramètre C<precise>.
