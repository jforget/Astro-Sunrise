...
faisant varier le midi solaire entre 11h et 13h. Mais on n'aurait rien vu
...
à la seconde évaluation du coucher du Soleil, 16:04:23.
...
Soleil se produit une fraction de seconde plus tard à 16:04:23. Il y a donc 3 h 59 mn 27 s où l'on utilise
...
=for html
<p>
<img src='sunset-slope.png' alt="Course du Soleil, comparaison entre Greenwich le 21/12 et la latitude 76 le 21/04" />
</p>
...
considérations que ci-dessus au sujet de la pente de la courbe s'appliquent.
...
de base utilise une vitesse moyenne de 0,986°/j, ce qui fait une erreur
de ±0,0308°/j. À l'occasion de la transition entre jour+nuit et le Soleil
...
R : L'algorithme précis est là pour contrebalancer le fait que la valeur
du midi solaire virtuel obtenue â la première itération peut se révéler
éloignée de la valeur finale à la convergence. Il peut y avoir presque 12 heures
entre le point temporel de calcul de la premiêre itération et le point
temporel à la convergence. À l'inverse, pour la transition avec
la nuit polaire, le premier midi solaire calculé est très proche
du lever ou du coucher du Soleil, donc du point de convergence.
L'écart sur la hauteur du Soleil sera très faible même si l'on utilise 
l'algorithme de base. Donc, il n'est pas vraiment utile de 
passer à l'algorithme précis.
...
une nuit où le point le plus bas du Soleil est 23°57' sous l'horizon et une nuit où ce même
point est à 24°3' ? Et il vaut mieux commencer à observer les astres lorsque
