...
Copyright (c) 2017, 2018 Jean Forget, tous droits réservés.
...
(L<https://metacpan.org>, par exemple)
...
=head2 Pour qui ? pour mon nounours

Avez-vous lu 
L<Le guide de brian pour résoudre tout problème Perl|http://articles.mongueurs.net/traductions/guide_brian.html> ?
...
L<parler à votre nounours|http://articles.mongueurs.net/traductions/guide_brian.html#item_En_avez_vous_parl__au_nounours_nbsp__>.
...
I<écrire> à mon nounours via GitHub.
...
de S<coordonnées :> azimuthales, équatoriales et écliptiques. Voici comment procéder pour
...
être compatible avec le paramètre C<alt> (hauteur) des fonctions de C<Astro::Sunrise>.
...
Si ce n'est pas encore fait, taper C<< <F2> >> pour avoir le menu de configuration.
Dans ce menu, choisir l'onglet C<Plugins>, puis C<Fuseau horaire>. Il est alors
possible de demander l'affichage de l'heure UTC au lieu du fuseau horaire
de celui du PC. Ne pas oublier de sauvegarder la configuration en sortant.
...
Prendre un lieu situé sur le méridien de Greenwich, ainsi le temps UTC
coïncidera avec le temps solaire moyen.

Dans de nombreux exemples ci-dessous, j'ai choisi de placer mon
observateur à 51° 28' 40" de latitude nord, 5" de longitude est et
S<27 m> de hauteur. La longitude et la latitude correspondent aux
coordonnées de l'observatoire de Greenwich données sur Wikipedia, la
hauteur est le reliquat du lieu précédemment utilisé.  Elle ne
correspond pas à l'altitude de l'observatoire, ou alors c'est une
...
Qui plus est, ce sont ce que j'appelerai des S<« mensonges> pour les S<adultes »>. Au début, on considérait
...
ni l'autre et de toutes façons, on s'en S<fiche. »> La vocation de la physique n'est
...
Autres mensonges pour les adultes, également appelés S<« hypothèses>
S<simplificatrices »>, utilisés dans ce S<texte :>
...
ils sont situés sur une sphère appelée S<« sphère> S<céleste »>,
...
mensonges. Toutefois, la plupart du temps, tous ces mensonges sont en vigueur.
...
et non pas en degrés. J'essaierai de donner les deux valeurs chaque fois que
j'en aurai besoin.
...
puis converties en coordonnées écliptiques.
  ...
  date       asc. droite  déclinaison  longitude écliptique
  4 janvier  18h59mn1s     -22°44'43"   -76°24'58" ou -76,4162°
  5 janvier  19h3mn24s     -22°38'18"   -75°23'58" ou -75,3996°
  3 juillet   6h48mn        22°58'35"   101°2'7"   ou 101,0355°
  4 juillet   6h52mn8s      22°53'39"   101°59'26" ou 101,9907°
...
par jour à l'apogée (ou aphélie).
...
11:43 et 12:15 du temps moyen, ici la variation est 8 fois plus importante,
faisant varier le midi solaire entre 10h et 14h. Mais on n'aurait rien vu
...
Cf. L<à la fin|"Analemme politiquement correcte"> pour une discussion politiquement
correcte de l'analemme et de la pseudo-analemme.

=head2 Principe du calcul itératif
...
heure 18:23:59. Puisque le midi solaire vrai a varié de 21 secondes en S<86 379 secondes>
(soit 1 jour moyen moins 21 secondes), par interpolation linéaire, à 18:23:59, soit 
S<23 225 secondes> plus tard le midi solaire moyen aura varié de 5,6 secondes.
De même, la hauteur, qui varie de 22'53" en S<86 379 secondes> aura varié de 6'9"
en S<23 225 secondes>. On obtient un midi solaire vrai virtuel de 11:56:28 et
...
18:23:59 et encore plus proche de la valeur donnée par S<Stellarium :> 18:23:24.

=head2 Mise en Œuvre de l'Algorithme Simple

Faisons d'abord une digression avec l'algorithme simple. Et au lieu de prendre 
comme exemple Greenwich, on prend le point symétrique de Greenwich par rapport
à l'équateur, pas très loin de l'Île Bouvet dans l'Atlantique sud. Et la date
choisie est celle du 4 janvier 2018. 
...
à anti-Greenwich, le midi solaire vrai se produit à 12:04:59 (en fait au
même instant qu'à Greenwich).

Puis on applique simultanément la rotation de la Terre sur elle-même (360,986 degrés par jour)
et la rotation d'un SVH (rappel : soleil virtuel homocinétique) autour de la Terre (0,986 degré par jour)
...
du Soleil est de 123,4° (123° 24' 25"). Donc il faut 8,2271 heures (8 h 13 mn 35 s)
pour parcourir cet angle et le coucher du SVH se produit à 20 h 18 mn 34 s.

=head2 Mise en Œuvre de l'Algorithme Précis

Avec l'algorithme précis, on dissocie la rotation de le Terre sur elle-même
(360,986 degrés par jour) et la rotation du Soleil autour de la Terre.
...
donne l'heure 20:17:31.

Cette valeur est moins bonne que celle de l'algorithme basique,
car elle ne tient pas du tout compte de la vitesse orbitale du Soleil
autour de la Terre, même approximativement. Mais nous allons nous rattraper avec
la seconde itération.

Pour la seconde itération, on détermine le midi solaire virtuel correspondant à la position du Soleil
à cette heure approximative 20:17:31. Ce midi solaire virtuel se produit à 12:05:06.
En partant de cette position, on applique la rotation de la Terre et on aboutit
à la seconde évaluation du coucher du Soleil, 20:17:24.

S'il y a besoin d'une troisième itération, on détermine le midi solaire virtuel
correspondant à la position du Soleil à 20:17:24. Ce midi solaire virtuel se produit à 12:05:06. Là encore
nous appliquons la rotation de la Terre, pour aboutir à une troisième évaluation
du coucher du Soleil, 20:17:24, différant de la valeur de la seconde itération 
par une fraction de seconde.

C'est une première façon de voir le S<calcul :> le Soleil anticipe sa position
...
par la première itération 20:17:31, le Soleil tourne à la vitesse réelle de 1,0166 degré par jour
en même temps que la Terre tourne sur elle-même à la vitesse de 360,986 degrés par jour.
Puis à 20:17:31, on ajuste en laissant le Soleil immobile et en faisant tourner la Terre seulement.
Dans cet exemple, on est arrivé un peu trop loin, donc la rotation de la Terre se fait à l'envers
en remontant le temps. Cela donne une nouvelle valeur 20:17:24 pour le coucher du Soleil.

Lors de la troisième itération, entre le midi solaire réel 12:04:56 et la valeur donnée
par la deuxième itération 20:17:24, le Soleil tourne à la vitesse réelle de 1,0166 degré par jour,
puis à 20:17:24, il se fige et ensuite on ajuste en faisant tourner la Terre seule sur elle-même. Le coucher du
Soleil se produit une fraction de seconde plus tard (ou plus tôt) à 20:17:24. Il y a donc 8 h 12 mn 28 s où l'on utilise
la vitesse réelle du Soleil et une fraction de seconde où l'on utilise une vitesse franchement erronée.
On y gagne par rapport au calcul simple qui utilise une vitesse légèrement erronée pendant toute
la période de 8 h, 12 mn et 28 s.
...
et le coucher du S<soleil ?> J'ai vu un algorihme qui ne demandait
...
visuel, l'algorithme est pourri. Mais je connais une personne qui utilise un algorithme
...
qe le ciel sera dégagé ou bien bouché par de gros nuages orageux, il faudra 
attendre l'heure dite ou allumer plus tôt.

Q : Je remarque que tu n'a pas utilisé les termes S<« avancer »>, S<« reculer »>
et S<« retarder »>.

R : Effectivement, j'ai un problème avec ce verbes. Lorsque l'on passe du 28/02/2015
au 28/02/2016, on I<avance> de 365 jours. Mais lorsque le coucher du soleil passe
de 17:35:43 à 17:37:29, il I<recule> ou il I<retarde> de 1 mn 46 s. Alors que la variation
est dans le même sens.

Q : Une autre remarque, comment as-tu fait pour avoir l'heure du coucher du Soleil
à la seconde près avec L<Astro::Sunrise> ?

R : Parce que je n'ai pas utilisé L<Astro::Sunrise>, mais
L<DateTime::Event::Sunrise>, qui fournit comme résultats des
objets de classe L<DateTime> pour lesquels le nombre de secondes est
défini.

Q : Et ne pourrait-on pas adapter L<Astro::Sunrise> pour fournir un
résultat C<"hh:mm:ss"> au lieu de simplement C<"hh:mm"> ?

R : Si, on pourrait. Mais cela a-t-il un S<sens ?> D'après Paul Schlyter, l'algorithme
...
On peut ainsi avoir une erreur de S<+45 s> le 28/02/2015 et une erreur de S<-50 s>
en octobre 2015 et en février 2050, mais pour les dates voisines au sens JJ/MM et au sens AAAA, l'erreur
sera quasiment celle du 28/02/2015. Peut-être S<+43 s> ou S<+46 s>, mais sûrement pas S<-50 s>
...
=head2 Analemme politiquement correcte
...
dans le sens des azimuths I<décroissants>.
...
Et les observateurs entre les deux S<tropiques ?> Un observateur
...
dos. Que faire S<alors ?> Il suffit de s'allonger au sol pour
